...
def thresholds_from_paper() -> dict:
    ...
    因此默认不判定, 只把实测仓位占比**记进 detail**(面板/日志里看得见)。
    要真正启用, 由运维显式给一个**同范畴**的下单前上限:

        PAPER["pretrade_max_position_pct"] = 0.30   # 或 PRETRADE_MAX_POSITION_PCT=0.30

    (0.30 的来由: 超过它就已经是"单票押注"量级而非等权组合, 且远高于 11.5% 的
    策略目标上限 —— 正常路径不可能触及, 故不会误杀。这是**运维可调**的显式选择,
    不是本模块替用户决定的数字。)
