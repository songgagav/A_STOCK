...
PAPER = {
    ...
    "pretrade_gates": True,
    # ---- 卖侧闸门: 账实相符金丝雀 (路线图 ⑮ 卖侧接线, 2026-09-22) ----
    # 买入侧的清单可以拒单(最坏是少建一个仓); 卖侧不行 —— 把卖出也拦掉等于把
    # 风险锁在仓里。故卖侧只查一件事: 引擎的"计划卖出量"与账本的"可卖量"是否
    # 一致。这两者由不同代码路径算出, 分叉时既不报错也看不出来。
    # 处置: 缩量到可卖成交 + 告警 + 落订单审计, **绝不取消卖出**。
    "sell_gate": True,
    # 容忍比例: 引擎与账本对"可卖"各算一次, 正常抖动只可能来自当日买入的整手
    # 取整(100 股, 相对单票持仓通常 <1%)。10% 留两个数量级余量: 正常抖动不告警,
    # 而"差一个数量级"的账实分叉必被抓到。
    "sell_cash_tolerance": 0.10,
    # ---- 日更里的回测/研究步骤窗口 (路线图 ⑨⑪⑭ 接线, 2026-09-22) ----
    # 三者都是"窗口越小越快"的滚动项, 累积成面板。0 = 关闭该步骤。
    # · portfolio_bt_days: 组合回测窗口(交易日)。组合结果的解读方式是"与单腿
    #   在同一撮合口径下的差", 窗口太短时该差被噪声主导, 10 天是"够看出量级、
    #   又不拖长日更"的折中。
    # · vnpy_regime_days: 多场景回测的**逐日重跑**天数(每个场景都要真跑一次 vnpy)。
    #   取 5 是性能取舍: 单次 vnpy 回测约 2~10s, 两场景 × 5 天 ≈ 20~100s。
    # · hypothesis_days: 因子假设巡检回看的交易日数(要覆盖前向收益的 hold=5 根)。
    #   60 天是 config.FACTOR_HYPOTHESIS_EVAL.min_obs_days 的下限, 两者必须
    ...
    # `exec_strategy.split_vs_unsplit` 已解析证明, 在线性冲击模型下拆单只增加
    # 执行风险(总冲击与档数无关, 而执行风险每档各付一次)。
    "exec_split": True,
    # 单笔名义额 / ADV 的上限。超过则**分摊到后续 tick**(不是取消) ——
    # 只降低单 tick 的吃单速度, 当日可达的成交量不变, 故不改变选股与权重结果。
    # 取值依据(诚实标注): 0.10 是机构执行的常见纪律量级(单笔不超过当日成交额的
    # 十分之一), 取它作为**保守起点**; 本仓虚拟盘当前**无真实成交数据**
    # (data/state.json.trades_history 为空), 故无法用成交反推, 待累积后重标。
    "participation_cap": 0.10,
    "exec_max_slices": 20,        # 单笔最多分几档(超过部分跨 tick 继续, 不放弃)
    "exec_tick_minutes": 30.0,    # 档间间隔(分钟), 用于折算执行窗口 T
    # ---- 为什么这里**没有**冲击/执行风险系数 ----
    # 走到这一步之前, 试过两版自建成本模型, 都被算术本身否掉了(详细推导见
    # `src/micro_cost.py` 的模块 docstring):
    #   · 用 `slippage_model.market_impact_rate`(临时项 0.5·p·vol): 乘上名义额后
    #     对订单规模是**二次的** => "拆 N 档"把总成本按 1/N 缩水, 模型自带
    #     "拆单更便宜"的结论(实测一笔 10 万元: 一次性 1902 元 / 拆 10 档 190 元)。
    #   · 换自建线性冲击 `k·p`: 若 k 是常数, 得到同一个"成本 ∝ 名义额²/ADV",
    #     还是"拆分即套利"; 若要让结论正确(拆单不改变总冲击), 费率就必须与
    #     规模无关 —— 那正是本仓现行常量口径。
    # 结论: **线性冲击模型在算术上无法回答"拆单省不省"** —— 它要么恒等,
    # 要么二次。真实冲击是次线性的(√p 量级), 但本仓**没有成交数据**可以标定
    # 那个指数(`data/state.json.trades_history` 为空), 所以不假装能算。
    # 于是本批次只做能算清的两件事: ①参与率闸门(真作用) ②成本恒等的对照留痕。
    # 将来有了成交, 用 `micro_cost.observed_cost_table()` 回归出规模弹性再回来填这里。
    # 是否把动态成本**计入撮合价**(改价格 => 改行为)。默认 False:
    # 本轮只做"计划 + 对照留痕", 不改变虚拟盘成交价 —— 与"不静默改变行为"一致。
    "dynamic_cost": False,
}

# ---- 因子假设的**收益评估门槛** (路线图 ⑭ 接线, 2026-09-22) ----
# `factor_hypothesis` 刻意只留 `return_evaluator` 回调, 并要求"生产接线处必须
# 显式传入自己的门槛"。本字典就是那份门槛 —— **故意没有内置默认值**:
# 收益门槛是整条流水线唯一的自由度, 一个写死的默认值会让"通过收益评估"
# 变成一句无法审计的话。`factor_hypothesis_eval.thresholds_from_config()` 在
# 缺键时**抛错**, 而不是悄悄用一个数糊过去。
#
# 取值依据(诚实标注): 下面三个数是**宽口径的初值**, 不是从本仓数据标定出来的。
#   · min_ic = 0.02  —— 个股日频 IC 达到 0.02 属可交易量级(业界经验区间
#     0.02~0.05); 取区间下沿, 因为这一层只是"收益不拖后腿", 方向与稳定性由
#     evidence 阶段负责。
#   · min_icir = 0.30 —— ICIR 是"稳定性"的度量, 0.3 相当于 IC 均值达到
#     0.3 倍 IC 标准差; 低于它说明 IC 靠少数几天撑着。
#   · min_obs_days = 60 —— 约一个季度交易日。样本更短时 ICIR 的抽样噪声
#     与 IC 本身同量级, 排序结果不可信。
# **标定方法**: 累积 `data/factor_hypotheses.jsonl` 里 verified 假设的真实
# IC/ICIR 分布后, 按分位数重设(与 method-1「阈值须基于下游表现」一致)。
FACTOR_HYPOTHESIS_EVAL = {
    "min_ic": 0.02,
    "min_icir": 0.30,
    "min_obs_days": 60,
