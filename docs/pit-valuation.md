...
4. ~~**滚动样本外验证**~~ **已完成（2026-09-13）**：`scripts/nonoverlap_rerun.py` 默认
   ...
   Sharpe 2.25±1.42，最深回撤 -14.78%），但过拟合检测 **17 通过 / 3 失败**。
   3 项 FAIL 中，#7（最优与中位数 Sharpe gap=2.14）是真实的窗口间离散度描述；
   #6（PBO=0.7308）与 #8（置换检验 p=0.9827）所依据的检验当前实现存在缺陷
   ——置换检验对"窗口汇总值"做 `mean` 置换在设计上恒等（均值是置换不变量，
   实测置换分布标准差仅 2.7e-16），PBO 由写死的噪声尺度 `0.5×std(Sharpe)` 主导，
   二者均不能作为过拟合证据，**在修正前不应用于资金可用性判定**。
