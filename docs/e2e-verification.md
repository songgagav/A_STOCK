...
第三阶段：全链路 dry-run（1天）
  [x] 一个完整交易日，盘前 -> 盘中 -> 盘后        -> ✅ **正常路径跑通**：`--date 2026-09-08` 命中
                                                  第②档、10 只、盘前 0.184s + 盘中 0.128s = **总 1.00s**；
                                                  生产状态全程未被触碰（详见下方观测与"设计缺陷"更正）
...
#### ⚠️ 我的第一版 dry-run 有一个**设计缺陷**（已修，并更正了由此得出的错误结论）

**缺陷**：把 `DATA_DIR` 重定向到空临时目录，会**连引擎要读的输入产物一起遮蔽** ——
`data/daily/<day>/selection.json` 与 `data/drl/<day>/target_plan.json` 都在 `DATA_DIR` 下。
空沙箱使 `load_targets()` 前四档**必然**取不到，于是落进第⑤档"现场选股"。

**由此得出的错误结论（我上一稿写过，现予更正）**：我曾据那次运行说"消费日取今天会落进第⑤档
现场选股" ⇒ **这是沙箱造成的假象，不是生产行为**。

- 实测生产数据：`daily/20260908`、`daily/20260907`、`drl/20260905` 的产物**都存在**；
- 按 `load_targets` 的档位顺序推演，2026-09-19 会命中**第④档（跨日 selection）**，
  **不会**落到第⑤档。
- 修法：dry-run 前把输入产物种进沙箱（`daily` ≈11.2 MB + `drl` ≈8.2 MB ≈19 MB），
  复制后写入（如 `save_selection`）也只落在沙箱。

#### 修正后的正常路径结果（`--date 2026-09-08`，已种入输入产物）

| 阶段 | 实测 |
|---|---|
| 盘前池构造 | **0.184 s**，10 只，`sel_day=20260908`，**档位 = `selection_same_day`（第②档）** |
| 盘中单次 tick | **0.128 s** |
| **总耗时** | **1.00 s** |
| 生产 `state.json` / `live_state.json` | **md5 与备份一致 ⇒ 未被触碰** |
| 临时目录产物 | 267 个 |

⇒ **正常路径是亚秒级的**，"09:25 前完成"在池命中的日子里毫无压力。

#### 第⑤档（现场选股）的真实测量——但要注意测量条件

上一版那次"墙钟 5.8 min / 累计 CPU 1406 s（≈4 核）/ 142 线程仍未返回"，**是在空沙箱条件下**
取得的。该条件在功能上**等价于"池产物全部缺失"**（正是第⑤档的触发条件），
因此**耗时数字本身仍是对第⑤档代价的有效测量**；但要如实标注：

- 它是**被迫**走到第⑤档的（因为沙箱里没有输入），**不是**生产在 2026-09-19 的自然行为；
- 我**没有取得它返回时的档位值**（在返回前中止），"第⑤档"是由日志特征
  （`[factor_health]` / `[units]` 等 selector 路径消息）推出的，非直接读到的 rung。

#### 与 09:25 缺口的关系（修正后的精确表述）

- ✅ **可以说**：正常路径亚秒级；风险**只集中在"前四档全失败"的日子**——此时会进第⑤档，
  而该档实测 ≥5.8 min、多核、**耗时上界未知且未受监控**。
- ⚠️ **不能说**：不能说"现场选股一定会跨过 09:25"。引擎 08:30 启动、到 09:25 有 ~55 min 预算。
- **正确结论**：`P0-FREEZE-0925`（09:25 冻结）必须与「池必达保障 + 选股耗时预算 + 逾期告警」
  三件事**一起**做。只加时间判断，会让超时完成的信号被直接丢弃或被静默接受，两种都不好。
