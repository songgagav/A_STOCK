...
⇒ 落进**第⑤档「现场选股」**。该档实测是**多核重计算**：进程 142 线程、**墙钟 5.8 min、
累计 CPU 1406 s（≈4 核）仍未完成**，临时目录尚无任何产物（此时我中止了它，以免长时间占 4 核）。

**这条观测与 09:25 缺口的关系（注意不要过度推断）**：

- ✅ **可以说的**：现场选股这条路径的**耗时上界未知且未受监控**——实测 ≥5.8 min 且尚未结束；
  在"当日池缺失"的日子里，**没有任何机制度量或保证它在 09:25 前完成**。
- ⚠️ **不能说的**：不能说"信号生成一定会跨过 09:25"。按 `daemon.py`，引擎 08:30 启动，
  到 09:25 有 ~55 min 预算；一个 ~10 min 的选股仍来得及。
- 因此正确的结论是：**09:25 冻结（`P0-FREEZE-0925`）与"池缺失时现场选股耗时不受控"是两个
  必须一起解决的问题** —— 光加"09:25 后不许变更"的时间判断，会让超时完成的信号被直接丢弃
  （或反之被静默接受），两种都不好。建议同时给出：池必达保障 + 选股耗时预算 + 逾期告警。
